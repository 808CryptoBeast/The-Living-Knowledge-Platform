...
function makeGoldCompassTexture(imgEl) {
  ...
  // Crop to square using the SHORTER dimension — this trims caption
  // strips that live in the extra height of a portrait-oriented image.
  ...
  // Centre the crop: skip any caption margins above/below (or left/right)
  ...
  // Clip to circle (0.488 = soft corner-rounding only, all compass content preserved).
  // Since we cropped to the shorter dimension above, the caption is already gone.
  // This clip just rounds off the square corners cleanly.
  ...
  circleCtx.save();
  circleCtx.beginPath();
  circleCtx.arc(size / 2, size / 2, size * 0.488, 0, Math.PI * 2, false);
  circleCtx.closePath();
  circleCtx.clip();
  ...
  specGrd.addColorStop(1,    "rgba(20,   10,   0, 0.28)");

  specCtx.save();
  specCtx.beginPath();
  specCtx.arc(size / 2, size / 2, size * 0.488, 0, Math.PI * 2, false);
  specCtx.closePath();
  specCtx.clip();
...
function makeCompassImageOverlay() {
  ...
  for (let i = 0; i <= 260; i++) {
    const a = (i / 260) * Math.PI * 2;
    rimPts.push(new THREE.Vector3(Math.cos(a) * rimR, Y + 0.09, Math.sin(a) * rimR));
  }

  const rim = new THREE.Line(
    new THREE.BufferGeometry().setFromPoints(rimPts),
    new THREE.LineBasicMaterial({ color: 0xffcc66, transparent: true, opacity: IS_MOBILE ? 0.28 : 0.38, depthWrite: false, depthTest: false, blending: THREE.AdditiveBlending })
  );
  rim.renderOrder = 21;
  compassGrp.add(rim);
...
function makeLights() {
  ...
  const key = new THREE.PointLight(0xfff4e8, IS_MOBILE ? 2.2 : 2.8, 200, 1.2); key.position.set(0, 40, 20);   scene.add(key);
